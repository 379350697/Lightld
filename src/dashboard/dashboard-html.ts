...
<body>
  <div class="app">
    ...
    <div class="position-section">
      ...
      <div class="position-card">
        <div class="card">
          <div class="card-title">活跃 Token</div>
          <div class="pos-mint" id="pos-mint">--</div>
          <div id="pos-symbol" style="font-size:14px;font-weight:600;margin-top:6px;">--</div>
        </div>
        <div class="card">
          <div class="card-title">入场资金</div>
          <div class="status-value" id="pos-entry-sol">--</div>
          <div class="status-label">SOL</div>
        </div>
        <div class="card">
          <div class="card-title">持仓时长</div>
          <div class="status-value" id="pos-hold-time">--</div>
          <div class="status-label" id="pos-opened-at">--</div>
        </div>
      </div>
    </div>
  ...
  <script>
    ...
    function renderStatus(data) {
      ...
      // Position
      $('#pos-mint').textContent = data.activeMint || '--';
      $('#pos-symbol').textContent = data.activeSymbol || '';
      $('#pos-entry-sol').textContent = typeof data.entrySol === 'number' ? formatSol(data.entrySol) : '--';

      if (data.openedAt) {
        const elapsed = Date.now() - new Date(data.openedAt).getTime();
        $('#pos-hold-time').textContent = formatDuration(elapsed);
        $('#pos-opened-at').textContent = '开仓: ' + formatTime(data.openedAt);
      } else {
        $('#pos-hold-time').textContent = '--';
        $('#pos-opened-at').textContent = '--';
      }
    }
    ...
    async function refreshAll() {
      const [status, pnl, orders, fills, incidents, logs] = await Promise.all([
